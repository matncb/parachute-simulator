...
class Solver:
    ...
    def solve(self, dt = 0.001, t_max = 1000, method = 'Radau', rtol = 1e-6, atol = 1e-8 ): 
        ...
        while t_start < t_max:
            sol = solve_ivp(
                func,  # Agora com assinatura correta (t, S)
                [t_start, t_max],
                S0,
                method=method,
                events=events,
                rtol=rtol,
                atol=atol,
                max_step=0.1  # Controla número de pontos
            )
            
            sols.append(sol)
            
            # Verifica qual evento terminou a integração
            if sol.t_events[0] and sol.t_events[0].size > 0:  # Chegou ao solo
                break
            elif sol.t_events[1] and sol.t_events[1].size > 0:  # Abertura drogue
                S0 = sol.y[:, -1]
                t_start = sol.t[-1]
                # Remove evento já ocorrido
                events = [ground_event, event_reefing, event_main]
            elif sol.t_events[2] and sol.t_events[2].size > 0:  # Abertura reefing
                S0 = sol.y[:, -1]
                t_start = sol.t[-1]
                # Remove evento já ocorrido
                events = [ground_event, event_main]
            elif sol.t_events[3] and sol.t_events[3].size > 0:  # Abertura main
                S0 = sol.y[:, -1]
                t_start = sol.t[-1]
                # Remove evento já ocorrido
                events = [ground_event]
